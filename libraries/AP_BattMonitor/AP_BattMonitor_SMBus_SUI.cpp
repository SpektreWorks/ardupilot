...
/*
  average over an array of the number of readable cells
 */
float AP_BattMonitor_SMBus_SUI::avg(const float *p) const
{
    float sum = 0;
    const uint8_t n = MIN(SUI_MAX_CELL_READ, cell_count);
...
void AP_BattMonitor_SMBus_SUI::timer()
{
    ...
    // read the manufacturers data for button press
    uint8_t mf_data[6];
    if (read_block(BATTMONITOR_SMBUS_MANUFACTURE_DATA, mf_data, sizeof(mf_data))) {
        bool pressed = (mf_data[1] >> 3) & 0x01;

        if (button_press_count >= BATTMONITOR_SMBUS_SUI_BUTTON_DEBOUNCE) {
...
bool AP_BattMonitor_SMBus_SUI::read_remaining_capacity()
{
    if (_params._pack_capacity > 0) {
        ...
        if (read_word(BATTMONITOR_SMBUS_REMAINING_CAPACITY, data)) {
            float consumed = _params._pack_capacity - data;
            if (consumed > 0 && consumed < _params._pack_capacity) {
